...
published: false
tags: weekendchallenge, devchallenge, ai, solana
cover_image: (upload assets/devto-cover.png)
---
...
**EMBER gives that abandoned passion a voice.** You confess what you quit. AI forges its persona — the dusty object, the game itself, or the younger you — and it *talks back*, out loud, in a voice matched to its temperament. It asks the question only it can ask: *why did you really stop?*
...
Every anonymized session joins the **Atlas of Abandoned Passions** — a live map of what humanity gives up, at what age, killed by what.
...
🎥 **Video:** (90s demo — real session, unscripted)

*(screenshots: conversation, the two doors, the Atlas, a pledge on Solana Explorer)*
...
The loop is **confess → converse → decide → commit → belong**, and each stage is one sponsor technology doing what it's uniquely good at:
...
The persona's temperament maps to a curated voice (wistful → Sarah, wry → George, bitter → Callum…). Every line the passion speaks arrives as real audio. Hearing your abandoned passion *say things out loud* is the difference between a chatbot and a séance.

### Snowflake — the Atlas
Every session lands in a Snowflake `sessions` table, and the Atlas page is pure live SQL: most-abandoned passions, abandonment reasons, dormancy years, rekindle rate. Snowflake is also the system of record for session state (the app runs serverless — persona and conversation context are reconstructed from Snowflake on every request).

### Solana — the commitment device
A pledge you can edit is a wish. Rekindle pledges and eulogy stones are attested on Solana devnet via the Memo program: `{kind, passion, commitment, confessionHash, ts}` — timestamped, unforgeable, permanent. No wallet needed: a server-side vault signs, so judges can go from confession to on-chain proof in one sitting.
